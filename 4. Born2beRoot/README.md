...
### Partitioning
The system is installed on a single disk (/dev/sda) with a dedicated boot partition and full-disk encryption for the rest. The encrypted partition (sda5_crypt) contains an LVM group that provides separate logical volumes for each major directory. This setup improves security, isolates system components, and makes storage management easier.
...
A strict password and authentication policy is enforced to increase system security.
...
* Minimum 2 days before it can be changed again
* 7-day warning before expiration
* Root must also follow the policy

**Sudo policy**
* Maximum 3 attempts before sudo fails
* Custom error message on wrong password
* Every sudo command is logged in `/var/log/sudo/`
* TTY mode required
* The sudo `secure_path` is restricted to approved system paths

### Services Installed

The system provides essential network and web services:
...
Defaults    badpass_message="bad password msg!"
```
